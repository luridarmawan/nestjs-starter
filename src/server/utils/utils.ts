...
export const dateFormat = (date, fstr, utc = false) => {
    ...
    return fstr.replace (/%[YmdHMS]/g, function (m) {
        switch (m) {
            case '%Y': return date[utcFunction + 'FullYear'] (); // no leading zeros required
            case '%m': m = 1 + date[utcFunction + 'Month'] (); break;
            case '%d': m = date[utcFunction + 'Date'] (); break;
            case '%H': m = date[utcFunction + 'Hours'] (); break;
            case '%M': m = date[utcFunction + 'Minutes'] (); break;
            case '%S': m = date[utcFunction + 'Seconds'] (); break;
